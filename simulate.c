      ...
      while(timer < 200)
	{
	  ...
	  for(i = 0; i < n; i++)
	      ...
	      if(active[i])
		{
		  ...
		  if(dx[i] == 0 && dy[i] == 0)
		    {
		      incx = (gsl_ran_gaussian(sqrt(2.*P.D))+ix*P.D)*scale[i];
		      incy = (gsl_ran_gaussian(sqrt(2.*P.D))+iy*P.D)*scale[i];
		    }
		  ...
		  if(_HARD_BOUNDARY) {
		    ...
		    if(x[i] < 0) x[i] = RND*5;
		    if(x[i] > cx) x[i] = cx-RND*5;
		    if(y[i] < 0) y[i] = RND*5;
		    if(y[i] > cy) y[i] = cy-RND*5;		    
		  }
		}
	    }
